...
using Firebase.SDK.Firebase.Storage.Models;
using Firebase.SDK.HttpClients.Storage;
using FirebaseCoreAdmin.Firebase.Storage;

namespace Firebase.SDK.Firebase.Storage;
...
internal class FirebaseStorage : IFirebaseStorage
{
    ...
    public Task<ObjectMetadata> GetObjectMetaDataAsync(string path)
    {
        ...
        var metaUri =
            new Uri($"{_configuration.StorageBaseAuthority2}/v1/b/{_credentials.GetDefaultBucket()}/o/{urlEncodedPath}",
                UriKind.Absolute);
    ...
    private void ValidateOptions(SigningOption options)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.Path)) throw new ArgumentNullException(nameof(options.Path));
    }
}